    ...
    def check_manifest_validity(self, manifest):
        ...
        for field in required_fields:
            if field not in manifest:
                raise ValueError(
                    f"{field} was not found in manifest for upgrade {self.__name}- aborting"
                )
    ...
    def check_required_files(self, manifest):
        ...
        for file in required_files:
            if not os.path.isfile(f"{job_path}{file}"):
                logging.info(
                    f"ERROR: Job {self.__name} is missing file {file} in path {job_path}"
                )
                # TODO: Maybe add ValueError here?
